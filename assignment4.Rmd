...
2. Line chart (20 points). The lines show the non-parametric association between age and monthly earnings for men and women.
    ...
    ```{r}
    Data %>%
        ggplot(aes(x = h_age_dv, y = h_payn_dv, linetype = sex_dv)) + geom_smooth(colour = "black") + coord_cartesian(xlim = c(15, 65), ylim = c(0, 2250)) +
...
4. Heat map (20 points).
    ...
    Data %>% 
        ...
        ggplot(aes(x = h_gor_dv, y = placeBorn)) + geom_tile(aes(fill = meanAGE)) +
        theme_classic() + 
